...
class FocalLoss(nn.Module):
    ...
    def forward(self,
        ...
        if self.use_sigmoid:
            ...
            alpha = self.alpha
            if isinstance(alpha, list) or isinstance(alpha, float):
                alpha = torch.tensor(alpha, dtype=torch.float32).to(x.device)
            else:
                alpha = None
            nll_loss = nn.NLLLoss(weight=alpha, reduction='none', ignore_index=ignore_index)
